 ...
 *
 * Prefers the sf-data360 transport from the sf-pi pi-package — it reuses the
 * same cached @salesforce/core Connection, path builder, and target-org /
 * api-version resolution that the d360_api LLM tool uses. Falls back to the
 * legacy `sf api request rest` shell-out when sf-pi is not installed or the
 * adapter failed to load. The mode chosen is surfaced in the TUI footer via
 * the transport label so the showcase is observable, not just internal.
 */
async function sfApi<T>(pi: ExtensionAPI, org: string, method: Method, path: string, body?: Json, signal?: AbortSignal): Promise<T> {
	...
	if (transport.info.mode === "sf-data360") {
		return transport.call<T>(pi, org, method, path, body, signal);
	}
	return sfApiCli<T>(pi, org, method, path, body, signal);
}

async function sfApiCli<T>(pi: ExtensionAPI, org: string, method: Method, path: string, body?: Json, signal?: AbortSignal): Promise<T> {
	const args = [
		"api",
		"request",
		"rest",
		`/services/data/v${API_VERSION}${normalizeApiPath(path)}`,
		"--target-org",
		org,
		"--method",
		method,
	...
	const result = await pi.exec("sf", args, { timeout: 120_000, signal });
	if (result.code !== 0) {
		throw new Error([result.stderr, result.stdout].filter(Boolean).join("\n").trim() || `sf exited ${result.code}`);
	}
...
class Spa<TObject, TField> implements Component {
	...
	private transportPill(): string | undefined {
		...
		const color = this.transportInfo.mode === "sf-data360" ? "borderAccent" : "warning";
